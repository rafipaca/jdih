<template>
  <footer class="bg-gray-100 py-8">
    <div class="container mx-auto px-4">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div class="text-center md:text-left">
          <h2 class="text-xl font-bold mb-4">Media Sosial</h2>
          <p class="text-gray-600 mb-4">
            Silakan kunjungi media sosial kami untuk mengetahui kegiatan JDIH terkini
          </p>
          <div class="flex justify-center md:justify-start space-x-4">
            <a href="#" class="text-green-700"><i class="fab fa-facebook fa-2x"></i></a>
            <a href="#" class="text-green-700"><i class="fab fa-twitter fa-2x"></i></a>
            <a href="#" class="text-green-700"><i class="fab fa-instagram fa-2x"></i></a>
            <a href="#" class="text-green-700"><i class="fab fa-youtube fa-2x"></i></a>
          </div>
        </div>
        <div class="text-center md:text-left">
          <h2 class="text-xl font-bold mb-4">Hubungi Kami</h2>
          <p class="text-gray-600">Sekretariat Daerah Provinsi Lampung</p>
          <p class="text-gray-600">Jl. WR.Mongonsidi No.69 Teluk Betung, Bandar Lampung</p>
          <p class="text-gray-600 mt-4"><i class="fas fa-phone-alt"></i> (0721) 481107</p>
          <p class="text-gray-600"><i class="fas fa-envelope"></i> birohukum@lampungprov.go.id</p>
          <p class="text-gray-600">
            <i class="fas fa-calendar-alt"></i> Senin - Jumat, 08:00 - 15:30 WIB
          </p>
        </div>
      </div>
      <div class="mt-8 text-center text-white bg-green-700 py-4">
        <p>
          Jaringan Dokumentasi dan Informasi Hukum | Kantor Biro Hukum Provinsi Lampung © 2024. All
          Rights Reserved.
        </p>
      </div>
    </div>
  </footer>
</template>

<script>
export default {
  name: 'Footer'
}
</script>

<style scoped>
/* Tambahkan gaya khusus di sini jika diperlukan */
</style>
